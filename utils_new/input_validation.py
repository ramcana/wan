...
class PromptValidator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.problematic_patterns = [
            ...
            (r'\b(kill|death|violence|blood|gore)\b', "VIOLENT_CONTENT", "Contains violent content that may be filtered"),
            (r'[^\x00-\x7F]{20,}', "EXCESSIVE_UNICODE", "Contains excessive non-ASCII characters"),
        ]
        ...
        self.model_requirements = {
            "t2v-A14B": {
                ...
                "avoid_patterns": ["static", "still image", "photograph"],
                "recommended_terms": ["moving", "flowing", "dynamic", "cinematic"]
            },
            "i2v-A14B": {
                "max_length": 256,
                "preferred_style": "motion description",
                "avoid_patterns": ["generate image", "create picture", "static"],
                "recommended_terms": ["motion", "movement", "animation", "transform"]
            },
            "ti2v-5B": {
                "max_length": 384,
                "preferred_style": "combined text-image description",
                "avoid_patterns": ["ignore image", "text only", "without image"],
                "recommended_terms": ["based on image", "following the scene", "continuing from"]
            }
        }
    ...
    def _suggest_optimizations(self, prompt: str, model_type: str, result: ValidationResult):
        ...
        video_terms = ["video", "motion", "movement", "animation", "sequence", "frames", "flowing", "dynamic"]
        ...
        model_config = self.model_requirements.get(model_type, {})
        recommended_terms = model_config.get("recommended_terms", [])
        has_recommended = any(term in prompt.lower() for term in recommended_terms)
        
        if not has_recommended and recommended_terms:
            result.add_info(
                f"Consider using terms like: {', '.join(recommended_terms[:3])}",
                "prompt",
                f"These terms work well with {model_type}"
            )
...
class ImageValidator:
    ...
    def _check_image_quality(self, image, result: ValidationResult):
        """Check image quality indicators"""
        try:
            import numpy as np
            ...
            if mean_brightness < 30:
                result.add_warning(
                    ...
                    "image",
                    "Consider using a brighter image for better video generation"
                )
            elif mean_brightness > 225:
                result.add_warning(
                    "Image appears very bright/overexposed",
                    "image",
                    "Consider using an image with better exposure"
                )
            ...
            if len(img_array.shape) >= 3:  # Color image
                ...
                if contrast < 20:
                    result.add_warning(
                        ...
                        "image",
                        "Consider using an image with more contrast for better results"
                    )
        
        except ImportError:
            # numpy not available, skip quality checks
            pass
        except Exception as e:
            logger.debug(f"Image quality check failed: {e}")
...
class ConfigValidator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.constraints = {
            ...
            "lora_strength": {"min": 0.0, "max": 2.0, "default": 1.0}
        }
        
        # Supported resolutions
        self.supported_resolutions = [
            "480p", "720p", "1080p", "square",
            "854x480", "1280x720", "1920x1080", "1024x1024"
        ]
        ...
        self.model_constraints = {
            ...
            "i2v-A14B": {
                ...
                "required_params": ["resolution", "steps", "strength"]
            },
            "ti2v-5B": {
                "max_steps": 70,
    ...
    def _check_required_params(self, params: Dict[str, Any], model_type: str, result: ValidationResult):
        ...
        required_params = self.model_constraints[model_type].get("required_params", [])
    ...
    def _validate_model_specific_params(self, params: Dict[str, Any], model_type: str, result: ValidationResult):
        ...
        if max_steps and isinstance(steps, (int, float)) and steps > max_steps:
            result.add_warning(
        ...
        if guidance and recommended_guidance and isinstance(guidance, (int, float)):
            if abs(guidance - recommended_guidance) > 2:
                result.add_info(
...
def validate_generation_request(prompt: str = None, image: Any = None, config: Dict[str, Any] = None, 
                              model_type: str = "t2v-A14B") -> ValidationResult:
    ...
    if config is not None:
        config_result = config_validator.validate_generation_params(config, model_type)
