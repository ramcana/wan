...
def test_backend_final():
    ...
    try:
        ...
        response = requests.post(f"{BASE_URL}/api/v1/prompt/enhance", json=data, timeout=10)
        if response.status_code == 200:
...
def show_working_commands():
    ...
    commands = [
        ...
        ("Prompt Enhancement", f'curl -X POST {BASE_URL}/api/v1/prompt/enhance -H "Content-Type: application/json" -d \'{{"prompt": "A beautiful landscape"}}\''),
        ("Generation (Form)", f'curl -X POST {BASE_URL}/api/v1/generation/submit -F "prompt=A cat in a garden" -F "model_type=t2v-A14B@2.2.0" -F "resolution=1280x720" -F "steps=20"'),
