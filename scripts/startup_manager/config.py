...
class ServerConfig(BaseModel):
    ...
    port: Optional[int] = Field(
        default=None, description="Server port (auto-assigned if None)"
    )
    auto_port: bool = Field(
        default=True,
        description="Automatically find available port if specified port is taken",
    )
    timeout: int = Field(
        default=30, ge=1, le=300, description="Server startup timeout in seconds"
    )
...
class FrontendConfig(ServerConfig):
    ...
    open_browser: bool = Field(
        default=True, description="Automatically open browser on startup"
    )
...
class RecoveryConfig(BaseModel):
    ...
    max_retry_attempts: int = Field(
        ...
        ge=1,
        le=10,
        description="Maximum retry attempts for recovery actions",
    )
    ...
    auto_kill_processes: bool = Field(
        default=False, description="Automatically kill conflicting processes"
    )
    fallback_ports: List[int] = Field(
        default=[8080, 8081, 8082, 3001, 3002, 3003],
        description="Fallback ports to try",
    )
...
class EnvironmentConfig(BaseModel):
    ...
    python_min_version: str = Field(
        default="3.8.0", description="Minimum required Python version"
    )
    node_min_version: str = Field(
        default="16.0.0", description="Minimum required Node.js version"
    )
    npm_min_version: str = Field(
        default="8.0.0", description="Minimum required npm version"
    )
    check_virtual_env: bool = Field(
        default=True, description="Check if running in virtual environment"
    )
    validate_dependencies: bool = Field(
        default=True, description="Validate that all dependencies are installed"
    )
    auto_install_missing: bool = Field(
        default=False, description="Automatically install missing dependencies"
    )
...
class SecurityConfig(BaseModel):
    ...
    allow_admin_elevation: bool = Field(
        default=True, description="Allow automatic admin elevation when needed"
    )
    firewall_auto_exception: bool = Field(
        default=False, description="Automatically add firewall exceptions"
    )
    trusted_port_range: tuple[int, int] = Field(
        default=(8000, 8010), description="Trusted port range for servers"
    )
    ...
    def validate_port_range(cls, v):
        if len(v) != 2 or v[0] >= v[1] or v[0] < 1024 or v[1] > 65535:
            raise ValueError(
                "trusted_port_range must be a tuple of (start, end) with start < end and both in range 1024-65535"
            )
        return v
...
class StartupConfig(BaseModel):
    ...
    retry_attempts: int = Field(
        ...
        ge=1,
        le=10,
        description="Number of retry attempts for failed operations (deprecated, use recovery.max_retry_attempts)",
    )
    retry_delay: float = Field(
        default=2.0,
        ge=0.1,
        le=30.0,
        description="Delay between retry attempts in seconds (deprecated, use recovery.retry_delay)",
    )
    verbose_logging: bool = Field(
        default=False,
        description="Enable verbose logging output (deprecated, use logging.level)",
    )
    auto_fix_issues: bool = Field(
        default=True,
        description="Automatically attempt to fix detected issues (deprecated, use recovery.enabled)",
    )
...
class ConfigLoader:
    ...
    def _validate_ports(self, results: Dict[str, Any]) -> None:
        ...
        if (
            self._config.backend.port
            and self._config.frontend.port
            and self._config.backend.port == self._config.frontend.port
        ):
            results["valid"] = False
    ...
    def _validate_environment_config(self, results: Dict[str, Any]) -> None:
        ...
        if not self._config.environment.check_virtual_env:
            results["info"].append("Virtual environment checking is disabled")
    ...
    def create_deployment_config(
        self, deployment_type: str = "production"
    ) -> Dict[str, Any]:
